...
# %% [markdown] id="415d830e" editable=true tags=["remove-cell"]
# **Setup**
#
...
# %% tags=["remove-cell"] outputId="7f0cd5ef-8faa-4fa9-a95f-fa95d055152c" colab={"base_uri": "https://localhost:8080/"} editable=true id="7e6514bb"
...
# %% [markdown] editable=true id="b8e7ac05"
# ```{warning}
# This chapter is a work in progress.
# ```

# %% [markdown] id="09a72a78" editable=true
# # Insights on urban development and sustainable mobility infrastructure across geographical scales
#
# * **Authors**: *[Sujit Kumar Sikder (IOER)](https://orcid.org/0000-0002-0265-7394)* *[Sukanto Das (ZALF)](https://orcid.org/0009-0003-6963-3834)*
...
# %% [markdown] id="86be5c96"
# ### 1. PART ONE
#
# Let's harvest indicators from the [World Bank API](https://data.worldbank.org/indicator) and look at them two ways: as a time series for Germany against the world regions, and as a globe for every country that reports.
#
# We will:
#
...
#
# The aim is to show how an open API can be **queried, verified and reused** — with no manual downloads, and nothing that store on disk space.

# %% [markdown] id="0eeb49c0"
# ### Why there is a need to integrated urban development and transportation?
#
# More than half the world now lives in cities, and transport/mobility sector is one of the largest sources of urban CO₂. A country can urbanise quickly while transport emissions stay small, or run a high transport share with barely any urban growth. What that relationship looks like depends on where you are — which is exactly what a global indicator can show.
#
# We use five World Bank indicators: three on urban development, two on CO₂ emissions.
...
# %% outputId="58ffb285-1714-4312-a197-7df7eebdd2f9" colab={"base_uri": "https://localhost:8080/"} id="c74788c2"
...
myplot = (
    ...
    + geom_point(
        ...
        aes(
            x="date",
            y="co2_transport_share",
            size="SP.URB.TOTL.IN.ZS"
        ),
        colour="#0072B2",   # blue
        alpha=0.6
    )
    ...
    + geom_line(
        panel,
        aes(x="date", y="EN.URB.LCTY.UR.ZS"),
        colour="#D55E00",   # orange
        size=1.5
    )
...
# %% colab={"height": 224, "base_uri": "https://localhost:8080/"} id="07bbe40c" outputId="b41b449e-130e-433c-f6f7-47f145db18ee"
...
# %% id="f786850f" outputId="5aa56c01-64c2-4ffd-da92-cd708ca63486" colab={"base_uri": "https://localhost:8080/"}
INDICATORS = {
...
# %% colab={"base_uri": "https://localhost:8080/"} outputId="c868135e-7331-41e0-8322-8a6a757ba7f1" id="e24cde1f"
COUNTRIES_URL = (
...
# %% [markdown] id="c1f6dfef"
# ## Conclusion
#
# This part of this chapter moves from:
#
...
# %%
## now follow PART 2: Urban Public Transit Frequency Indicator — Germany
